...
export default function OrbCanvas() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Build equirectangular canvas texture with PUMP image tiled 4x2
      ...
      const ring = (r: number, th: number, col: number, op: number, rx: number, ry: number, rz: number) => {
      ...
      const r1 = ring(1.44, .009, 0x0099ff, .85, Math.PI / 3, 0, 0);
      const r2 = ring(1.72, .007, 0x00ccff, .5, Math.PI / 1.6, Math.PI / 4, 0);
      const r3 = ring(1.58, .005, 0x0055cc, .32, 0, 0, Math.PI / 2.5);
      const r4 = ring(1.88, .004, 0x003388, .2, Math.PI / 2.2, Math.PI / 3, Math.PI / 5);
      const r5 = ring(2.05, .003, 0x0022aa, .12, Math.PI / 4, Math.PI / 5, 0);
      ...
      let omx = 0, omy = 0;
      const onMove = (e: MouseEvent) => { omx = (e.clientX / innerWidth - .5) * .9; omy = (e.clientY / innerHeight - .5) * .9; };
      ...
      const animate = () => {
        ...
        r3.rotation.x += .006; r3.rotation.z -= .004; r4.rotation.y += .003; r4.rotation.z += .002;
      ...
      return () => { cancelAnimationFrame(raf); document.removeEventListener("mousemove", onMove); renderer.dispose(); };
    };
  ...
  return <canvas ref={ref} style={{ width: 520, height: 520, maxWidth: "88vw", display: "block" }} />;
}
