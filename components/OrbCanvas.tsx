...
export default function OrbCanvas() {
  ...
  const initGlobe = () => {
    ...
    // canvas has CSS width/height 520px so clientWidth = 520 immediately
    ...
    [[1.07, 0x0044bb, 0.09], [1.28, 0x002266, 0.04], [1.04, 0x0099ff, 0.13]].forEach(([r, col, op]) =>
    ...
    const mkRing = (r: number, th: number, col: number, op: number, rx: number, ry: number, rz: number) => {
      const m = new THREE.Mesh(new THREE.TorusGeometry(r, th, 2, 100),
        new THREE.MeshBasicMaterial({ color: col, transparent: true, opacity: op }));
      m.rotation.set(rx, ry, rz); scene.add(m); return m;
    };
    const r1 = mkRing(1.44, 0.009, 0x0099ff, 0.85, Math.PI/3, 0, 0);
    const r2 = mkRing(1.72, 0.007, 0x00ccff, 0.5,  Math.PI/1.6, Math.PI/4, 0);
    const r3 = mkRing(1.58, 0.005, 0x0055cc, 0.32, 0, 0, Math.PI/2.5);
    const r4 = mkRing(1.88, 0.004, 0x003388, 0.2,  Math.PI/2.2, Math.PI/3, Math.PI/5);
    const r5 = mkRing(2.05, 0.003, 0x0022aa, 0.12, Math.PI/4, Math.PI/5, 0);
    ...
    for (let i = 0; i < 22; i++) {
      ...
      dots.push({ m: d, a: (i / 22) * Math.PI * 2, sp: 0.006 + Math.random() * 0.007,
        rad: 1.46 + Math.random() * 0.22, yz: Math.random() * 0.55 - 0.27, ph: Math.random() * Math.PI * 2 });
      scene.add(d);
    }

    let omx = 0, omy = 0;
    window.addEventListener("mousemove", (e) => {
      omx = (e.clientX / innerWidth  - 0.5) * 0.9;
      omy = (e.clientY / innerHeight - 0.5) * 0.9;
    });
    ...
    const animate = () => {
      ...
      pl1.position.x = 2.2 + Math.sin(t * 0.5); pl1.position.y = 2 + Math.cos(t * 0.4);
  ...
  useEffect(() => {
    if ((window as any).THREE) initGlobe();
  ...
  return (
    <>
      ...
      <div style={{ position: "relative", display: "inline-block" }}>
        <canvas
          ref={ref}
          style={{ width: CANVAS_SIZE, height: CANVAS_SIZE, maxWidth: "88vw", display: "block" }}
        />
        {[
          { text: "TOKEN-2022", style: { top: "8%",  right: "-4%"  } },
          { text: "18.4B SUPPLY", style: { bottom: "16%", left: "-6%"  } },
          { text: "X1 CHAIN",   style: { top: "48%", right: "-10%" } },
        ].map(({ text, style }) => (
          <div key={text} style={{
            position: "absolute", padding: "6px 14px",
            background: "rgba(4,14,24,.9)", border: "1px solid rgba(0,170,255,.25)",
            fontFamily: "var(--font-mono)", fontSize: ".68rem", color: "var(--blue)",
            whiteSpace: "nowrap", animation: "dpFloat 4s ease-in-out infinite",
            ...style,
          }}>{text}</div>
        ))}
      </div>
